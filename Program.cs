...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "3":
            for (int i = 0; i < maxPets; i++)
            {
                //this block checks fi the animal has a valid age
                if (ourAnimals[i, 0] != "ID #: " && ourAnimals[i, 2] == "Age: ?")
                {
                    do
                    {
                        Console.WriteLine($"Enter a valid pet's age for {ourAnimals[i, 0]}: ");
                        readResult = Console.ReadLine();
                        if (readResult != null)
                        {
                            animalAge = readResult;
                ...
                //this block checks if the animal has a valid physical description
                if (ourAnimals[i, 0] != "ID #: " && (ourAnimals[i, 4] == "Physical description: " || ourAnimals[i, 4] == "Physical description: tbd"))
                {
                    do
                    {
                        Console.WriteLine($"Enter a valid pet's physical description for {ourAnimals[i, 0]}: ");
                        readResult = Console.ReadLine();
                        if (readResult != null)
                        {
            ...
            System.Console.WriteLine("Checking done... Press enter to continue.");
            readResult = Console.ReadLine();
            break;
        ...
        case "4":
            for (int i = 0; i < maxPets; i++)
            {
                //this block checks fi the animal has a nickname
                if (ourAnimals[i, 0] != "ID #: " && (ourAnimals[i, 3] == "Nickname: " || ourAnimals[i, 3] == "Nickname: tbd"))
                {
                    do
                    {
                        Console.WriteLine($"Enter a valid pet nickname for {ourAnimals[i, 0]}: ");
                        readResult = Console.ReadLine();
                        if (readResult != null)
                        {
                ...
                if (ourAnimals[i, 0] != "ID #: " && (ourAnimals[i, 5] == "Personality: " || ourAnimals[i, 5] == "Personality: tbd"))
                {
                    do
                    {
                        Console.WriteLine($"Enter a valid pet's personality for {ourAnimals[i, 0]}: ");
                        readResult = Console.ReadLine();
                        if (readResult != null)
                        {
            ...
            Console.WriteLine("Checking nickname and personality done...");
            Console.WriteLine("Press the Enter key to continue.");
            readResult = Console.ReadLine();
            break;
...
case "5":
    ...
    string animalInputID;
    do
    {
        Console.Write("Type the animal ID you wish to edit the age: ");
        animalInputID = Console.ReadLine() ?? "";
    } while (string.IsNullOrWhiteSpace(animalInputID));

    string formattedID = "ID #: " + animalInputID;
    bool foundID = false;

    for (int i = 0; i < maxPets; i++)
    {
        if (ourAnimals[i, 0] == formattedID)
        {
            foundID = true;
            ...
            do
            {
                Console.Write($"Enter a valid pet age for {formattedID}: ");
                inputAge = Console.ReadLine() ?? "";
            } while (!int.TryParse(inputAge, out newAge));

            ourAnimals[i, 2] = "Age: " + newAge.ToString();
            Console.WriteLine($"Age updated successfully for {formattedID}.\n");
            break; // já encontramos e atualizamos, não precisa continuar o loop
        }
    }
...
case "6":
    ...
    do
    {
        Console.Write("Type the animal ID you wish to edit the personality: ");
        inputID = Console.ReadLine() ?? "";
    } while (string.IsNullOrWhiteSpace(inputID));

    formattedID = "ID #: " + inputID;
    ...
    for (int i = 0; i < maxPets; i++)
    {
        if (ourAnimals[i, 0] == formattedID)
        {
            ...
            do
            {
                Console.Write($"Enter the new personality for {formattedID}: ");
                newPersonality = Console.ReadLine() ?? "";
            } while (string.IsNullOrWhiteSpace(newPersonality));

            ourAnimals[i, 5] = "Personality: " + newPersonality;
            Console.WriteLine($"Personality updated successfully for {formattedID}.\n");
            break;
        }
    }
...
case "7":
    ...
    do
    {
        Console.Write("Enter the characteristic to search for (e.g., 'calmo', 'ativo'): ");
        characteristic = Console.ReadLine() ?? "";
    } while (string.IsNullOrWhiteSpace(characteristic));

    bool foundAny = false;

    Console.WriteLine($"\nCats with the characteristic \"{characteristic}\":\n");

    for (int i = 0; i < maxPets; i++)
    {
        if (ourAnimals[i, 0] != null && ourAnimals[i, 0].StartsWith("ID #: c"))
        {
            // Verifica se a característica está na posição 5
            if (ourAnimals[i, 5] != null && ourAnimals[i, 5].ToLower().Contains(characteristic.ToLower()))
            {
                foundAny = true;
                Console.WriteLine("----------------------");
                for (int j = 0; j < ourAnimals.GetLength(1); j++)
                {
                    if (ourAnimals[i, j] != null)
...
case "8":
    ...
    do
    {
        Console.Write("Enter the characteristic to search for in dogs (e.g., 'calmo', 'ativo'): ");
    ...
    for (int i = 0; i < maxPets; i++)
    {
        if (ourAnimals[i, 0] != null && ourAnimals[i, 0].StartsWith("ID #: d"))
        {
            if (ourAnimals[i, 5] != null && ourAnimals[i, 5].ToLower().Contains(dogCharacteristic.ToLower()))
            {
                foundDog = true;
                Console.WriteLine("----------------------");
                for (int j = 0; j < ourAnimals.GetLength(1); j++)
